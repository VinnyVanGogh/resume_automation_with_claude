...
from .exceptions import ConversionError, FileError, ProcessingError, ValidationError
from .types import (
...
class ResumeConverter:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            raise ConversionError(
                f"Failed to initialize pipeline components: {e}", stage="initialization"
            )
    ...
    def convert(
        ...
        result = ConversionResult(
            success=True, input_path=input_path, processing_time=0.0
        )
        ...
        try:
            ...
            self._report_progress(
                "complete", 100.0, "Conversion completed successfully"
            )
    ...
    def convert_batch(
        ...
        batch_result = BatchConversionResult(total_files=len(input_paths))
            ...
            for i, input_path in enumerate(input_paths):
                ...
                try:
                    ...
                    failed_result = ConversionResult(
                        success=False, input_path=Path(input_path)
                    )
    ...
    def _validate_outputs(
        self, output_files: list[Path], result: ConversionResult
    ) -> None:
